...
def _validate_run_inputs(file_a: Optional[str], file_b: Optional[str], out_path: Optional[str], env_file: Path, provider: str, model: str, timeout: Optional[int]) -> None:
    ...
    # NOTE: .env file check removed - ACM2 injects keys via environment variables
    # _read_key_from_env_file checks os.environ first, then falls back to .env file
...
def run(file_a: Optional[str] = None,
    ...
    try:
        ...
        _uuid = None
    # Use ACM-provided run_id from environment if available, otherwise generate short id for standalone use
    ...
    # ===== NEW METERING HOOK =====
    # Emit structured metering event with full usage extraction
    # This is NEW code - does not replace existing logging, just adds to it
    try:
        from metering.event_builder import MeteringEventBuilder
        
        metering_builder = MeteringEventBuilder(
            provider=provider_name,
            model=cfg.get("model") or "unknown",
            run_id=run_id,
            phase=os.environ.get("FPF_PHASE", "fpf.generate"),
        ...
        # Emit to log file (always) and ACM endpoint (if configured)
        metering_builder.emit(metering_event)
        LOG.info("[METERING] Event emitted: attempt_id=%s cost=%s", 
                 metering_event.get("attempt_id", "?")[:8], 
                 metering_event.get("cost", {}).get("total", "?"))
    except Exception as metering_err:
        LOG.warning("[METERING] Failed to emit metering event (non-fatal): %s", metering_err)
    # ===== END METERING HOOK =====
